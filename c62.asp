...
.Button03{
	BORDER: 0px; BACKGROUND: url("Image/bg06_register.gif") no-repeat; WIDTH: 110px; HEIGHT: 25px; cursor:hand;cursor:pointer;
}
-->
</style>
<link href="css/c61_style.css" rel="stylesheet" type="text/css">
<script src="js/jquery-1.4.1.min.js" language="javascript"></script>
<script src="js/c62_js.js" language="javascript"></script>
...
<form name="form1" method="post" action="">
  <table width="570" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td height="367" align="right" background="image/bg_register.jpg"><table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0">
        <tr>
        ...
        <tr align="center">
          <td height="50" colspan="2"><input name="b1" type="submit" class="Button03" id="b1" value=" ">
&nbsp;&nbsp;
          ...
          <td height="50" colspan="2">
		    ...
		    <%
		  	if request.form.count<>0 then
				...
				sql="select * from 用户表 where 用户名='"&a1&"'"
				set rs=conn.execute(sql)
				if rs.eof then
					sql="insert into 用户表 (用户名,密码) values ('"&a1&"','"&a2&"')"
					conn.execute(sql)
					response.write("<marquee>用户注册成功！</marquee>")
				else
					response.write("用户名："&a1&"已经存在！")
				end if	
				conn.close()
			end if 
		  %>
